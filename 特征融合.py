...
class MultiHeadAttention(nn.Module):
    def __init__(self, feature_dim, num_heads):
        ...
        self.attention_heads = nn.ModuleList([
            Attention(feature_dim) for _ in range(num_heads)
        ])
...
def attention(x_train, x_test, N, head):
    X_train_tensor = torch.tensor(x_train, dtype=torch.float32)
    X_test_tensor = torch.tensor(x_test, dtype=torch.float32)
    # 初始化注意力模型,N是维数
    attention_model = MultiHeadAttention(N, head)
    # 使用注意力机制进行特征融合
    with torch.no_grad():
        X_train_fused, _ = attention_model(X_train_tensor)
        X_test_fused, _ = attention_model(X_test_tensor)
    # 将融合后的特征转换为numpy数组
    X_train_fused = X_train_fused.numpy()
    X_test_fused = X_test_fused.numpy()
...
def main(path):
    ...
    #data = data.drop(['社交型', '传统型','判断-观察','亲和力'], axis=1)
    ...
    x_train1, x_test1, y_train, y_test = train_test_split(data, binary_labels, test_size=0.3, random_state=58)

    x_train, x_test = attention(x_train1.values, x_test1.values, len(x_train1.columns), 3)
    ...
    model_forest = MultiOutputClassifier(RandomForestClassifier(n_estimators=100, random_state=7,max_depth=7))
    ...
    # 获取每个标签的概率预测
    probabilities = model.predict_proba(x_test)  # X_new是新样本
